...
Requisito SEC (política de fair access):
    ...
    Alternativa: ``cazador.sec_user_agent`` en config.yaml.
...
def _get_headers(config: dict) -> dict[str, str]:
    ...
    Prioridad: variable de entorno ``SEC_USER_AGENT`` > ``cazador.sec_user_agent``.

    Raises:
        ValueError: Si no hay User-Agent configurado.
    """
    ua = os.environ.get("SEC_USER_AGENT", "").strip()
    if not ua:
        ua = str(config.get("cazador", {}).get("sec_user_agent") or "").strip()
    if not ua:
        raise ValueError(
            "La SEC requiere un User-Agent con nombre y email. "
            "Define SEC_USER_AGENT=\"TuNombre email@dominio.com\" en .env "
            "(o cazador.sec_user_agent en config.yaml)."
        )
